...
#define MAX_TIMEOUT 2 // 2ms
bool timeOut(bool reset = false){
	static long mils = 0;
	if(reset){
		mils = millis();
		return false;
	...
	return ((millis()-mils) > MAX_TIMEOUT);
}
...
*************************************************************************/
bool i2c_start_wait(unsigned char address)
{
	timeOut(true);
...
{	
	...
	// check value of TWI Status Register. Mask prescaler bits
	twst = TW_STATUS & 0xF8;
	if( twst != TW_MT_DATA_ACK && !timeOut()) return false;
	return !timeOut();
